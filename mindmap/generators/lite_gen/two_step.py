...
from mindmap.generators.lite_gen.constants import (
    OLD_PROMPT_MAP,
    QA_PROMPT_MAP,
    STRUCTURE_PROMPT_MAP,
)
from mindmap.generators.lite_gen.exceptions import AnswerCountMismatchError
from mindmap.generators.lite_gen.models import (
    AnswerList,
    MindmapStructure,
    NodeAnalysisResult,
    RootLabel,
)
from mindmap.generators.lite_gen.prompts import (
    NODE_ANALYSIS_PROMPT,
    ROOT_DETERMINATION_PROMPT,
)
from mindmap.generators.lite_gen.simple import (
    MULTIMODAL_CATEGORIES,
    TEXT_CATEGORIES,
    GeneratorConfig,
    SimpleGenerator,
)
from mindmap.generators.lite_gen.utils.image_helpers import enforce_image_limit
...
from mindmap.generators.univ_gen.core.actions.docs import fetch_all_docs_content
from mindmap.generators.univ_gen.core.stages import DocHandler
...
class TwoStageGenerator(SimpleGenerator):
    ...
    async def _prepare_document_inputs(
        ...
        result = await doc_handler.chunk_docs(docs_and_their_content)
        if result is None:
            raise ValueError("Chunking result is None")

        chunked_docs_df, _ = result
    ...
    async def _generate_structure(
        self,
        human_message_content: List[str | Dict[str, Any]],
    ...
    async def _fetch_answers_for_question_list(
        ...
        """
        Stage 2 (Robust): Fetches answers.
        If RAG_INITIAL_BATCH_SIZE is set and smaller than the question
        count, it immediately splits processing into batches.
        Otherwise, it attempts to process all at once, falling back to
        batching on failure.
        """
        ...
        initial_batch_size_env = int(EnvConsts.ADVANCED_BATCH_SIZE)

        should_run_monolithic = True
        if 0 < initial_batch_size_env < len(questions):
            should_run_monolithic = False
            logger.info(
                f"Input size ({len(questions)}) exceeds batch limit "
                f"({initial_batch_size_env}). "
                "Skipping monolithic attempt and processing in batches "
                "immediately."
            )

        answers = None

        if should_run_monolithic:
            try:
                ...
                if answers is not None:
                    duration = time.time() - start_time
                    logger.info(
                        "Stage 2: Successfully fetched all answers in a "
                        f"single batch in {duration:.2f}s."
                    )
                    yield answers
                    return

                logger.warning("Batch stream finished without answers.")

            except Exception as e:
                logger.warning(
                    f"Stage 2: Full batch processing failed after {max_retries} "
                    f"attempts ({e}). Switching to batched strategy."
                )

        target_batch_size = (
            initial_batch_size_env
            if not should_run_monolithic
            else self.FALLBACK_BATCH_SIZE
        )
        ...
        fallback_stream = self._handle_cascading_fallback(
            ...
            user_instructions,
            batch_size_override=target_batch_size,
        )
        ...
        duration = time.time() - start_time
        logger.info(
            "Stage 2: Answer fetching completed via batching in "
            f"{duration:.2f}s."
        )
    ...
    async def _execute_llm_batch_request(
        ...
        last_exception = RuntimeError("Operation failed unexpectedly.")
        for attempt in range(max_retries + 1):
            try:
                raw_output = None
                content_chunks = []
                            ...
                            if event["event"] == "on_chat_model_stream":
                                ...
                                if chunk_text:
                                    ...
                                    if current_answer_count > answer_count:
                                        ...
                                        global_count = (
                                            progress_offset + answer_count
                                        )

                                        yield StatusChunk(
                                            title="Generating content "
                                            f"({global_count}/{self.num_nodes})"
                                            "..."
                                        )
                    else:
                ...
                if content_chunks:
                    ...
                    if isinstance(raw_output, AnswerList):
                        ...
                        raise ValueError(
                            "LLM output was an unexpected type: "
                            f"{type(raw_output)}"
                        )
    ...
    async def _handle_cascading_fallback(
        ...
        batch_size = batch_size_override or self.FALLBACK_BATCH_SIZE

        # Level 1 Fallback: Mini-batches
        batches = self._chunk_list(questions_with_indices, batch_size)
        for i, batch in enumerate(batches):
            batch_questions = [q for _, q in batch]
            batch_indices = [idx for idx, _ in batch]

            current_progress_offset = len(final_answers)
            ...
            try:
                batch_stream = self._execute_llm_batch_request(
                    ...
                    user_instructions,
                    max_retries=1,
                    progress_offset=current_progress_offset,
                )
                ...
                if batch_answers and len(batch_answers) == len(batch_indices):
                    ...
                    # If answers are missing or mismatched, mark entire
                    # batch for one-by-one fallback
                    failed_in_batch.extend(batch)

            except Exception as e:
                logger.warning(
                    f"Batch {i + 1} failed: {e}. Marking questions for "
        ...
        if failed_in_batch:
            logger.info(
                f"Processing {len(failed_in_batch)} remaining questions "
                f"one-by-one."
            )
            for original_index, question_text in failed_in_batch:
                ...
                try:
                    ...
                    logger.error(
                        "Single question fallback failed for index "
                        f"{original_index}: {e}"
                    )
    ...
    async def _determine_root_node(
        ...
        try:
            ...
            if isinstance(raw_output, RootLabel):
    ...
    async def generate(
        ...
        if answers is None:
            logger.error(
                ...
                "No answers received (result was None)."
            )
            return
