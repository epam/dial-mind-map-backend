...
class PPTXChunker(BaseDocChunker):
    ...
    async def chunk(self, doc_with_content: DocAndContent) -> list[Chunk]:
        ...
        pdf_bytes = await _convert_pptx_to_pdf(doc_with_content.content)
        if not pdf_bytes:
            return []

        # 2. Create a new DocAndContent item with the PDF content
        pdf_item = DocAndContent(doc=doc_with_content.doc, content=pdf_bytes)
        ...
        for chunk in chunks:
            chunk.doc_category = DocCat.PPTX  # type: ignore
...
class WholePPTXChunker(BaseDocChunker):
    ...
    async def chunk(self, doc_with_content: DocAndContent) -> list[Chunk]:
        ...
        for chunk in chunks:
            chunk.doc_category = DocCat.PPTX_AS_A_WHOLE  # type: ignore
...
class UnstructuredPPTXChunker(BaseDocChunker):
    ...
    async def chunk(self, doc_with_content: DocAndContent) -> list[Chunk]:
        ...
        if not isinstance(doc_with_content.content, bytes):
            logger.warning(
                f"Expected bytes for PPTX {doc_with_content.doc.id}, got non-bytes."
            )
            return []

        try:
            # 1. Asynchronously convert PPTX slides to PIL images
            slide_images = await convert_pptx_to_images(
                doc_with_content.content
            )
        except Exception as e:
            logger.error(
                f"Failed to convert PPTX to images for doc {doc_with_content.doc.id}: {e}",
                exc_info=True,
            )
            return []

        # 2. Pass content and images to the synchronous processing
        # function in a thread
        pages = await aio.to_thread(
            self._extract_and_combine, doc_with_content.content, slide_images
        )

        if not pages:
            logger.warning(
                f"Chunking resulted in 0 pages for doc {doc_with_content.doc.id}."
            )
            return []
        ...
        return self._create_chunks_from_page_groups(
            merged_slide_groups, doc_with_content.doc
        )
    ...
    async def to_markdown(self, doc_with_content: DocAndContent) -> str:
        ...
        content_bytes = doc_with_content.content

        def sync_process_pptx(b_content: bytes):
            try:
                elements = partition_pptx(file=io.BytesIO(b_content))
                return "\n\n".join(
        ...
        return await aio.to_thread(sync_process_pptx, content_bytes)
