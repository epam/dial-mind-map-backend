...
class EdgeProcessor:
    ...
    def _get_similarity_df(
        ...
        """
        embeddings = np.stack(concept_df[Col.EMBEDDING].tolist())
        ...
        if not cos:
            ...
            for i in range(n):
                # noinspection PyArgumentList
                distances_i, indices_i = index.search(embeddings[i : i + 1], n)  # type: ignore
    ...
    def _adj_root_out_edges(
        ...
        if num_needed < 0:
            ...
            if num_more_needed > 0:
                root_similarities: pd.Series = similarity_df.loc[root_id]  # type: ignore
    ...
    async def strong_con_graph(
        ...
        new_edge_df.index = pd.Index(
            range(max_idx + 1, max_idx + 1 + len(new_edge_df))
        )
