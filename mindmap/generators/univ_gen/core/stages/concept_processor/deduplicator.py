...
class ConceptDeduplicator:
    ...
    async def deduplicate(self, data: RawMindMapData) -> RawMindMapData:
        ...
        root_id = data.root_index if data.root_index is not None else -1
        ...
        for _ in range(10):  # Max 10 iterations of deduplication.
            ...
            groups, group_indices, _, root_group_id = self._get_dup_groups(
                active_df, root_id
            )

            if groups is None or group_indices is None:
            ...
            concept_df = utils.add_concepts(
                concept_df, cast(list[dict[Any, Any] | str], new_concepts)
            )
        ...
        root_candidates = concept_df.loc[root_mask & active_mask].index.values
        new_root_index = (
            int(root_candidates[0]) if len(root_candidates) > 0 else root_id
        )
    ...
    async def _run_deduplication_chains(
        ...
        root_group_id: int | None,
    ) -> tuple[Sequence[Any], Optional[RootDeduplicationResult]]:
        """
