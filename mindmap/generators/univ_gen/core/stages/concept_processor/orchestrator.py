...
class ProcessingOrchestrator:
    ...
    async def define_root(self, data: RawMindMapData) -> RawMindMapData:
        ...
        qapair_name_to_id = cast(
            dict[str, int],
            pd.Series(root_df.index, index=root_df[Col.NAME]).to_dict(),
        )
        ...
        concept_df = utils.add_concepts(
            concept_df, cast(list[dict[Any, Any] | str], sub_roots)
        )
    ...
    async def wrap_up(
        ...
        if root_index is None:
            raise ValueError("Root index could not be determined.")
...
class AddConceptOrchestrator(ProcessingOrchestrator):
    ...
    async def _define_root_w_old_root(
        ...
        old_root_concept = (
            data.old_root_concept
            if data.old_root_concept is not None
            else pd.DataFrame()
        )
