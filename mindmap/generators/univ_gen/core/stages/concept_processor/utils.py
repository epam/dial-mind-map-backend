...
from mindmap.generators.univ_gen.chainer import Embedder
...
from mindmap.generators.univ_gen.core.utils.constants import FACT, SOURCE_IDS
...
def get_concept_id_changes(concept_df: pd.DataFrame) -> dict[int, list[int]]:
    ...
    # Cast is necessary because pandas .to_dict() returns dict[Hashable, Any]
    return cast(
        dict[int, list[int]],
        grouped.apply(lambda group: list(group.index)).to_dict(),
    )
...
def make_unique_names(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for name in duplicates:
        ...
        if num_active == 1 and inactive_mask.any():
            # Pandas stub issue: df.loc is typed as Scalar, but here
            # it's a string Series
            df.loc[inactive_dup_mask, Col.NAME] += "_"  # type: ignore
        elif num_active > 1:
            ...
            df.loc[name_mask, Col.NAME] += "_"  # type: ignore
    return df
...
def prepare_new_concept_from_dict(
    ...
    validates `source_ids`, aggregates `flat_part_ids` from all
...
def update_rels(
    ...
    for _, row in rel_df.iterrows():
        origin_id = int(cast(Any, row.at[Col.ORIGIN_CONCEPT_ID]))
        target_id = int(cast(Any, row.at[Col.TARGET_CONCEPT_ID]))
