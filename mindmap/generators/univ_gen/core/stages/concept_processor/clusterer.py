...
from mindmap.generators.univ_gen.chainer import ChainCreator, ChainRunner
from mindmap.generators.univ_gen.chainer.utils.constants import ChainTypes as Ct
...
from mindmap.generators.univ_gen.common.constants import FieldNames as Fn
from mindmap.generators.univ_gen.core.structs import RawMindMapData
from mindmap.generators.univ_gen.core.utils.constants import (
    ClusteringMethods as Cm,
)
from mindmap.generators.univ_gen.core.utils.constants import (
    FrontEndStatuses as Fes,
)
from mindmap.generators.univ_gen.core.utils.constants import Pi
from mindmap.generators.univ_gen.core.utils.frontend_handler import put_status
...
class ConceptClusterer:
    ...
    async def create_hierarchy(self, data: RawMindMapData) -> RawMindMapData:
        ...
        chunk_len = len(chunk_df) if chunk_df is not None else 0
        is_graph_trivial = chunk_len <= 1 or len(concept_df) < 4
    ...
    async def _execute_clustering_strategy(
        ...
        while True:
            ...
            # FIX: Convert Series to list for np.stack
            embedding_matrix = np.stack(lvl_qapair_df[Col.EMBEDDING].tolist())
    ...
    @staticmethod
    def _det_max_cluster_size(chunk_df: pd.DataFrame | None) -> int:
        """
        ...
        if (
            chunk_df is None
            or chunk_df.empty
            or Col.CONCEPT_IDS not in chunk_df.columns
        ):
            return 5
    ...
    def _get_concepts_from_clusters(
        cls,
        cluster_synth_results: Sequence,
        lvl: int,
    ...
    async def _process_synth_clusters(
        ...
        qapairs: list[dict[Any, Any] | str] = cls._get_concepts_from_clusters(
        ...
        rel_name_to_id_map = cast(
            dict[str, int],
            pd.Series(lvl_active_df.index, index=composite_key).to_dict(),
        )
...
class AddConceptClusterer(ConceptClusterer):
    ...
    async def _execute_clustering_strategy(
        ...
        concept_df: pd.DataFrame,
        relation_df: pd.DataFrame,
        max_chunk_size: int,  # FIX: Renamed from max_cluster_size to match base class
    ) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        """
        ...
        for lvl in sorted_levels:
            ...
            embedding_matrix = np.stack(
                current_processing_df[Col.EMBEDDING].to_list()
            )
            ...
            _, clusters = self._create_lvl_clusters(
                embedding_matrix,
                indices_arr,
                max(max_chunk_size, 10),  # FIX: using renamed arg
            )
