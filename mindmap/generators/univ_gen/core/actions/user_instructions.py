...
from mindmap.generators.base import MMRequest
from mindmap.generators.univ_gen.adapter import GMContract as Gmc
from mindmap.generators.univ_gen.chainer import ChainCreator as Cc
from mindmap.generators.univ_gen.chainer import ChainRunner as Cr
from mindmap.generators.univ_gen.chainer import ChainTypes as Ct
from mindmap.generators.univ_gen.chainer.model_handler import ModelCreator
from mindmap.generators.univ_gen.chainer.prompt_generator.constants import (
    PromptInputs as Pi,
)
from mindmap.generators.univ_gen.chainer.response_formats import (
    RefinedStyleResult,
...
async def _extract_text_from_documents(
    ...
    del_docs_list = getattr(request, "del_documents", [])

    if del_docs_list:
        del_ids = {doc.id for doc in del_docs_list}
        documents = [doc for doc in request.documents if doc.id not in del_ids]
    else:
    ...
    chunk_result = await doc_handler.chunk_docs(docs_and_their_content)

    if chunk_result is None:
        return {}

    chunked_docs_df, _ = chunk_result
...
async def process_user_style(
    ...
    if not user_prompt.strip():
        ...
        default_style_prompt = _format_directives_as_string(
            cast(Any, request_default_directives)
        )
    ...
    if is_final:
        ...
        result_prompt = _format_directives_as_string(cast(Any, result))
