...
from mindmap.generators.univ_gen.core.stages.doc_handler.structs import (
    DocAndContent,
)
from mindmap.generators.univ_gen.core.utils.file_storage import read_file_bytes
...
async def fetch_all_docs_content(
    ...
    file_group_task = aio.gather(
        *[_fetch_file_content(doc, file_storage) for doc in file_docs]
    )

    # Await both groups
    results = await aio.gather(link_task, file_group_task)

    # Unpack explicitly typed results
    link_results = cast(list[DocAndContent], cast(object, results[0]))
    file_results = cast(list[DocAndContent], cast(object, results[1]))

    # Combine them
    all_results = link_results + file_results
    ...
    if successful_fetches:
        ...
        logger.info(
            f"Successfully fetched content for: {successful_docs_names}"
        )
...
async def _fetch_file_content(
    ...
    try:
        ...
        if byte_content is None:
            logger.warning(f"No content found in storage for {doc.id}")
