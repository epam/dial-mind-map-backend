...
from mindmap.generators.base import EdgesFile, GraphFiles, NodesFile
from mindmap.generators.univ_gen.adapter import GMContract as Gmc
from mindmap.generators.univ_gen.adapter import (
    GraphFilesAdapter,
    translate_graph_files,
)
...
from mindmap.generators.univ_gen.common.exceptions import ApplyException
from mindmap.generators.univ_gen.core.structs import MindMapData
from mindmap.generators.univ_gen.core.utils.constants import DefaultValues as Dv
from mindmap.generators.univ_gen.core.utils.constants import EdgeWeights as Ew
from mindmap.generators.univ_gen.core.utils.constants import Patterns as Pat
from mindmap.generators.univ_gen.core.utils.misc import is_valid_uuid
...
def process_graph(
    ...
    ids_to_replace = pd.isna(numeric_ids)
    new_id_inc = 0
...
def _filter_by_document_id(
    ...
    while True:
        ...
        for idx_raw, row in new_node_df.iterrows():
            idx = cast(int, idx_raw)
            ...
            for fact in facts_list:
                source_ids_raw = fact[Fn.SOURCE_IDS]

                assert isinstance(source_ids_raw, list)
                source_ids = source_ids_raw

                initial_source_count = len(source_ids)
            ...
            if not filtered_facts:
                ...
                new_node_df.at[idx, Col.ANSWER] = cast(Any, filtered_facts)

                if len(filtered_facts) < original_fact_count:
                    new_node_df.at[idx, Col.MODIFIED] = 1

            if idx not in iteration_drops:
                if Col.CITATION in row.index and row[Col.CITATION] is not None:
                    ...
                    new_node_df.at[idx, Col.CITATION] = cast(
                        Any, filtered_citations
                    )
