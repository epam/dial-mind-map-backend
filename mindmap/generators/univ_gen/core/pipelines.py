...
from ..common import type_vars as tv
...
class PipelineHandler:
    ...
    async def generate(
        ...
        req: InitMindmapRequest = (
            request
            if request is not None
            else cast(InitMindmapRequest, self.request)
        )

        try:
            docs_content = await fetch_all_docs_content(
                req.documents, self.file_storage
            )
            ...
            safe_instructions = cast(
                dict[str, Union[str, dict[str, str]]], self.filter_instructions
            )

            extract_task = asyncio.create_task(
                extractor.extract_mindmap_elements(
                    chunk_df, flat_part_df, safe_instructions
                )
            )
            ...
            if concept_df is None or relation_df is None or chunk_df is None:
                raise EmptyDataException("Extractor returned incomplete data")
    ...
    async def delete(
        ...
        try:
            ...
            final_root_id = root_id if root_id is not None else 0

            yield MindMapData(
                node_df=node_df, edge_df=edge_df, root_id=final_root_id
            )
        finally:
            cancel_tasks(tasks)
    ...
    async def add(
        ...
        try:
            ...
            if concept_df is None or relation_df is None:
                raise EmptyDataException(
                    "Extractor returned no concepts or relations for add"
                )
    ...
    async def _yield_graph_nodes(
        ...
        if node_df is None or node_df.empty:
            if not has_problematic_nodes:
                ...
                safe_root_id = str(EMPTY_NODE_DATA.id or "")
                yield RootNodeChunk(root_id=safe_root_id)
            return
