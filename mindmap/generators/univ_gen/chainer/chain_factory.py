...
from mindmap.generators.univ_gen.common.exceptions import GeneratorException
...
class ChainRunner:
    ...
    async def _run_chain(
        ...
        with get_openai_callback() as cb_handler:
            final_callbacks.append(cb_handler)

            config: RunnableConfig = {"callbacks": final_callbacks}
        ...
        for attempt in range(max_retries + 1):
            ...
            try:
                ...
                if attempt < max_retries and retry_field_name:
                    ...
                    error_types = cast(
                        set[str], field_error_config.get(Fv.ERROR_TYPES, set())
                    )
                ...
                if e.completion and e.completion.choices:
                    ...
                    logger.warning(
                        f"Token usage for this attempt: {usage_info}"
                    )
                else:
                    logger.warning(
