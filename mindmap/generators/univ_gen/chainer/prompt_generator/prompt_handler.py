...
class PromptHandler(ABC):
    ...
    ) -> list[SystemMessage | HumanMessage | SystemMessagePromptTemplate]:
        """Generates a dynamic list of messages from inputs."""
        raise NotImplementedError(
            f"{cls.__name__} is designed for static prompt "
    ...
    def get_cpt(cls) -> ChatPromptTemplate:
        ...
        raise NotImplementedError(
            f"{cls.__name__} is designed for dynamic message "
    ...
    def _build_system_message_content(cls) -> str:
        ...
        for part_type, part_key_or_none in cls._system_message_parts:
            ...
            try:
                ...
                if part_type == Ppt.EXAMPLE and isinstance(part_content, str):
                    part_content += "\n"
...
class _BaseHumanMessageBuilder(PromptHandler):
    ...
    def _get_input_wrapper(cls) -> dict[str, str]:
        ...
        return cast(
            dict[str, str],
            load_prompt_part(Ppt.INPUT_WRAPPER, cls._input_wrapper_key),
        )
...
class MultimodalExtractionBase(_BaseExtractionHandler):
    ...
    def get_pt_gen(
        ...
        return [system_msg, HumanMessage(content=cast(Any, human_content))]
...
class _BaseExampleInjectorSystemBuilder(_BaseSystemMessageBuilder):
    ...
    def _build_system_message_content(cls) -> str:
        ...
        raw_steps = cast(
            dict[str, str],
            load_prompt_part(Ppt.STEPS_DESC, cls._steps_desc_key),
        )
        ...
        return "\n".join(filter(None, [str(p) for p in parts]))
