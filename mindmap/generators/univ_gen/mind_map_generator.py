...
class MindMapGenerator(Generator):
    ...
    @handle_exceptions_and_logs  # type: ignore
    async def generate(
    ...
    async def _stream_deletions(
        ...
        request = cast(ApplyMindmapRequest, pipeline_handler.request)

        is_just_del = bool(request.add_documents)
        async for item in pipeline_handler.delete(
            request.del_documents, mind_map_data, is_just_del
        ):
            yield item
    ...
    async def _stream_additions(
        ...
        if mind_map_data.node_df.empty:
            gen_request = InitMindmapRequest(documents=request.add_documents)
            async for item in pipeline_handler.generate(gen_request):
                yield item
        else:
            async for item in pipeline_handler.add(
                request.add_documents, mind_map_data
            ):
                yield item
