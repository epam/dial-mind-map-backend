...
# 'mindmap' package. It is the single source of truth for all
# hardcoded strings.
class GMContract:
    """Defines the data contract keys for mindmap objects."""
...
class _GMNode(base.Node):
    data: base.NodeData = Field(alias=GMContract.DATA_KEY)

    model_config = ConfigDict(populate_by_name=True, from_attributes=True)


class _GMEdge(base.Edge):
    data: base.EdgeData = Field(alias=GMContract.DATA_KEY)

    model_config = ConfigDict(populate_by_name=True, from_attributes=True)


class _GMNodesFile(base.NodesFile):
...
class _GMEdgesFile(base.EdgesFile):
    edges: list[_GMEdge] = Field(alias=GMContract.EDGE_KEY)

    model_config = ConfigDict(populate_by_name=True, from_attributes=True)


class GMGraphFiles(base.GraphFiles):
...
try:
    from mindmap.utils.graph_patch import embeddings_model

    GM_AVAILABLE = True
except ImportError:
    _ERROR_MESSAGE = "The 'mindmap' package is required..."
...
class GeneralMindmapAdapter(AbstractAdapter):
    """The concrete adapter for the 'mindmap' backend."""
    ...
    def __init__(self):
        if not GM_AVAILABLE:
            raise ImportError(
                "Cannot instantiate GeneralMindmapAdapter: 'mindmap' "
                "package not found."
            )

    def get_embeddings_model(self) -> EmbeddingModel:
        return cast(EmbeddingModel, embeddings_model)
