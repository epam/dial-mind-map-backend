...
from mindmap.models.graph import Edge, Graph, GraphData, Node
...
def get_subgraph(
        ...
        for index, nodeId in enumerate(foundPathToPreviousNode):
            ...
            if index > 0:
                ...
                foundEdges = [
                    ...
                    and edge.data.source == target
                    and edge.data.target == source
                ]
                if not foundEdges:
                    foundEdges = [
                        edge
    ...
    while not currentLevelQueue.empty() and len(resultNodes) < maxNodes:
        while not currentLevelQueue.empty():
            ...
            if id not in nextNeighbourIndex:
                nextNeighbourIndex[id] = 0

            neighborIndex = nextNeighbourIndex[id]
            ...
            if id in adjacencyList and neighborIndex < len(adjacencyList[id]):
                ...
                nextNeighbourIndex[id] = neighborIndex + 1

            if id not in adjacencyList or nextNeighbourIndex[id] >= len(
                adjacencyList[id]
            ):
                del nextNeighbourIndex[id]
            else:
    ...
    for edge in edges:
        if not isinstance(edge.data, Edge):
            continue
        ...
        if (
            ...
            and not any(
                ...
                if isinstance(addedEdge.data, Edge)
            )
        ):
