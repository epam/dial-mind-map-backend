...
from mindmap.models.graph import GraphData
from mindmap.utils.logger_config import logger
from mindmap.utils.rag_eval_utils import (
    TokenUsageByModelsCallback,
    calc_token_usage_costs,
    token_usage_context,
)
from mindmap.utils.tokens_queue import TokensQueue
    ...
    for result in await batch_file_reader.read():
        ...
        for i in doc_url_to_ref[file_name]:
            if i["chunk_id"] >= len(result[1]["chunks"]):
                ...
                if not record or len(record.chunks) <= chunk_id:
                    logger.warning(
...
async def run_chain(
    ...
    if os.getenv("IS_EVAL_TEST_RUN", "0") == "1":
        ...
        content_dump = full_content.model_dump() if full_content else {}
        choice.set_state({**content_dump, "priced_usage": priced_usage_dict})

        response_text = full_content.response_text if full_content else ""

        # Build and add references attachment
        refs = json.dumps(
            await build_references(response_text, docs, nodes, client)
        )
        ...
        if attachment_graph:
            ...
            if data and isinstance(data, list) and "data" in data[0]:
                data[0]["data"]["details"] = response_text
