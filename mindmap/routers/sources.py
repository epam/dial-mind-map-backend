...
from mindmap.completion.dial_rag_integration import build_index
from mindmap.config import DIAL_URL
from mindmap.dial.batch_file_reader import BatchFileReader, BatchRawFileReader
from mindmap.dial.batch_file_writer import BatchFileWriter
from mindmap.dial.client import DialClient, Lock
from mindmap.generators.base import Document
...
from mindmap.pptx.converter import convert_pdf_to_images, convert_pptx_to_images
...
async def add_file_source_background(
    ...
    try:
        ...
        status = "Not enough tokens for indexing"
        record = None
    ...
    chunk_docs_result = await doc_chunker.chunk_docs(
        docs_and_their_content or []
    )
    chunk_df = chunk_docs_result[0] if chunk_docs_result else None
...
async def delete_source(request: Request, source_id: str):
    ...
    async with (
        await DialClient.create(DIAL_URL, request) as client,
        client.create_lock(f"sources/{source_id}"),
    ):
        if client._metadata.documents_file is None:
            raise HTTPException(404, "Not found")
