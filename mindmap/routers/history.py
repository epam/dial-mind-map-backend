...
from mindmap.models.metadata import History, HistoryItemType
from mindmap.routers.sources import migrate_sources_from_old_format
...
async def undo(
    ...
    if left_idx < 0 or right_idx >= len(history.steps):
        raise HTTPException(400, "Impossible to undo. History is empty")
    if history.steps[right_idx].user != user:
    ...
    for step in range(right_idx, left_idx - 1, -1):
...
async def redo(
    ...
    for step in range(left_idx, right_idx + 1):
        for changed_file in history.steps[step].changes or []:
            sources_related = sources_related or changed_file.type in [
                HistoryItemType.SOURCES,
                HistoryItemType.SOURCE_STATE,
...
async def undo_redo(request: Request, action: str):
    ...
    async with await DialClient.create(DIAL_URL, request) as client:
        if action == "undo":
            left_idx = right_idx = client._metadata.history.current_step
            while (
                left_idx >= 0
                and left_idx < len(client._metadata.history.steps)
                and client._metadata.history.steps[left_idx].user == user
                and client._metadata.history.steps[left_idx].skip
            ):
                left_idx -= 1

            related_to = await undo(
                left_idx, right_idx, client, client._metadata.history, user
            )
            client._metadata.history.current_step = left_idx - 1
        else:
            left_idx = right_idx = client._metadata.history.current_step + 1
            while (
                right_idx + 1 < len(client._metadata.history.steps)
                and client._metadata.history.steps[right_idx + 1].user == user
                and client._metadata.history.steps[right_idx + 1].skip
            ):
                right_idx += 1
            ...
            client._metadata.history.current_step = right_idx
