...
from mindmap.models.attachment import GRAPH_CONTENT_TYPE
from mindmap.models.graph import Graph
from mindmap.utils.cards_retriever import create_cards_retriever
from mindmap.utils.graph_patch import GraphPatcher
from mindmap.utils.logger_config import logger
from mindmap.utils.match import (
...
def create_router_chain(
    ...
    def router_chain(inputs: Dict):
        ...
        guardrail_llm_checker = cast(BaseChatModel, llm).with_structured_output(
        ...
        if os.getenv("PROJECT_INSTRUCTIONS", "0") == "1":
            from mindmap.completion.project_instructions import (  # type: ignore[reportMissingImports]
                CHAT_BOT_NAME,
                CHECKER_PROMPT,
                CUSTOM_INSTRUCTIONS,
                DOMAIN,
        ...
        conditional_guardrail_chain = RunnableBranch(
            (
                lambda x: cast(dict, x).get("chat_guardrails_enabled", False),
                guardrail_checker_chain,
            ),
        ...
        if inputs.get("matched_node", None):
            return (
                ...
                | RunnablePassthrough.assign(
                    out_of_scope_reasoning=lambda x: cast(dict, x)[
                        "guardrail_decision"
                    ].reasoning
                )
                | RunnableBranch(
                    (
                        lambda x: cast(dict, x)[
                            "guardrail_decision"
                        ].out_of_scope,
...
def format_final_eval_response(inputs: dict) -> dict:
    ...
    """
    guardrail_decision: OutOfScopeCheckerResponse | None = inputs.get(
        "guardrail_decision"
    )
